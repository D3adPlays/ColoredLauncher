...
if (
	...
	//@ts-expect-error
	api.log(
		"Username is already set, setting it to the field: " +
...
function playButton() {
	let username = (
		document.getElementById("usernameField") as HTMLInputElement
	).value;
	//@ts-expect-error
	...
	if (!usernameValidator.test(username)) {
		...
		Toastify({
			...
			style: {
				background:
					"linear-gradient(90deg, rgba(144,12,12,1) 0%, rgba(253,29,29,1) 100%)",
			},
			stopOnFocus: true,
		}).showToast();
		return;
...
function getServerStatus() {
	const apiUrl = "https://api.minetools.eu/ping/play.coloredcity.fr/25565";
	window
		.fetch(apiUrl)
		...
		.then(function (data) {
			if (data.error) {
                document.getElementById("svIndicator")!.innerHTML = "HORS LIGNE";
			} else {
				document.getElementById("svIndicator")!.innerHTML = "<strong>EN LIGNE" + " " + data.players.online + "/" + data.players.max + "</strong>";
			}
		})
